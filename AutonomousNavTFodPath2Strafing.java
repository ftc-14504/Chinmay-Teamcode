package org.firstinspires.ftc.teamcode.AutonomousNavTFodPaths;
...
@Autonomous(name="AutonomousNavTFodPath2Strafing", group="Autonomous")

public class AutonomousNavTFodPath2Strafing extends LinearOpMode {
    ...
    public void strafeDrive(double speed,
        ...
        if (opModeIsActive()) {
            ...
            newLeftTarget = left.getCurrentPosition() - (int) ((inches - 0.3) * COUNTS_PER_INCH);
            newLeft2Target = left2.getCurrentPosition() + (int) ((inches - 0.3) * COUNTS_PER_INCH);

            newRightTarget = right.getCurrentPosition() + (int) ((inches - 0.3) * COUNTS_PER_INCH);
            newRight2Target = right2.getCurrentPosition() - (int) ((inches - 0.3) * COUNTS_PER_INCH);
    ...
    public void initDrive() {
        ...
        collector = hardwareMap.get(Servo.class, "hinge");
        slider = hardwareMap.get(DcMotor.class, "lift");
    ...
    public void runOpMode() {
        ...
        /*
        sliderTarget = slider.getCurrentPosition() + (int) ((sliderInches - 0.5) * COUNTS_PER_INCH);


        slider.setMode(DcMotor.RunMode.RUN_TO_POSITION);


        slider.setTargetPosition(sliderTarget);

        slider.setMode(DcMotor.RunMode.RUN_USING_ENCODER);

        slider.setPower(0.5);
        */
        //strafeDrive(DRIVE_SPEED, -2, -2, 5.0,-1);
        ...
        while (opModeIsActive() && !amIDone) {
            ...
            for (int i = 0; i < filteredMinerals.size(); i++) {
                if ((filteredMinerals.get(i).getRight() - filteredMinerals.get(i).getLeft() < 100) || (filteredMinerals.get(i).getBottom() < 925)) { //if the size of the box is less than 100, then it's in the background so its removed
            ...
            if (filteredMinerals != null && turnPos < 2) {
            ...
            do {
                ...
                if (!targetVisible && triggered) {
                    if (strafeCount < 5 * 3) {
                        strafeDrive(0.25, -3, 4);
                        strafeCount += 3;
                    }
                }
            }
            while (!targetVisible && triggered) ;
            ...
            if (targetVisible) {
                switch (picture_found) {
                    case 0:
                        ...
                        turnDegrees(10);
                        strafeDrive(0.5, -strafeCount - 53.5, 6);
                        depositMarker();
                        strafeDrive(0.6, 53.5, 6);
                        turnDegrees(90);
                        moveForward(30);
                        amIDone = true;
                        break;
                    ...
                    case 2: // front craters: So we are in blue alliance, depot to the right, crater to the right back]
                        ...
                        //moveForward(-3);
                        turnDegrees(10);
                        strafeDrive(0.5, 53.5 + strafeCount, 6);
                        /*moveForward(43.5);
                        turnDegrees(-165);//could comment this out for red crater*/
                        depositMarker();

                        strafeDrive(0.6,-24, 6);
                        //turnDegrees(-150);
                        //moveForward(30);
                        amIDone = true;
                        break;
...
//turnDegrees(5);
                        ...
                        if(strafe_check % 4 == 0)
                        {
                            strafeDrive(DRIVE_SPEED, -1, -1, 5.0,-1);
                        }else if(strafe_check % 4 == 1)
                        {
                            strafeDrive(DRIVE_SPEED, -1, -1, 5.0,1);
                        }else if(strafe_check % 4 == 2)
                        {
                            strafeDrive(DRIVE_SPEED, -1, -1, 5.0,1);
                        }
                        else if(strafe_check % 4 == 3)
                        {
                            strafeDrive(DRIVE_SPEED, -1, -1, 5.0,-1);
                        }
                        strafe_check++;
